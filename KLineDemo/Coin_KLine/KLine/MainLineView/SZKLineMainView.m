...
@interface SZKLineMainView()

/**
 *  需要绘制的model数组
 */
@property (nonatomic, strong) NSMutableArray <SZKLineModel *> *needDrawKLineModels;
...
@implementation SZKLineMainView
...
- (void)drawRect:(CGRect)rect{
    ...
    CGContextFillRect(context, CGRectMake(0, SZKLineMainViewMaxY, self.frame.size.width, self.frame.size.height - SZKLineMainViewMaxY));
    ...
    if(self.MainViewType == SZKLineCenterViewTypeKline)
    {
        SZKLine *kLine = [[SZKLine alloc]initWithContext:context];
        kLine.maxY = SZKLineMainViewMaxY;
        
        [self.needDrawKLinePositionModels enumerateObjectsUsingBlock:^(SZKLinePositionModel * _Nonnull kLinePositionModel, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        [self.needDrawKLinePositionModels enumerateObjectsUsingBlock:^(SZKLinePositionModel * _Nonnull positionModel, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        [self.needDrawKLinePositionModels enumerateObjectsUsingBlock:^(SZKLinePositionModel * _Nonnull positionModel, NSUInteger idx, BOOL * _Nonnull stop) {
            ...
            CGPoint drawDatePoint = CGPointMake(point.x + 1,SZKLineMainViewMaxY + 1.5);
    ...
    if (self.targetLineStatus == SZKLineTargetLineStatusBOLL) {
        // 画BOLL MB线 标准线
        MALine.MAType = SZBOLL_MB;
        MALine.BOLLPositions = self.BOLL_MBPositions;
        [MALine draw];
        
        // 画BOLL UP 上浮线
        MALine.MAType = SZBOLL_UP;
        MALine.BOLLPositions = self.BOLL_UPPositions;
        [MALine draw];
        
        // 画BOLL DN下浮线
        MALine.MAType = SZBOLL_DN;
        MALine.BOLLPositions = self.BOLL_DNPositions;
        [MALine draw];
        
    } else if ( self.targetLineStatus != SZKLineTargetLineStatusCloseMA){
...
- (CGFloat)getExactXPositionWithOriginXPosition:(CGFloat)originXPosition{
    ...
    NSInteger startIndex = (NSInteger)((xPositoinInMainView - self.startXPosition) / ([SZKLineGlobalVariable kLineGap] + [SZKLineGlobalVariable kLineWidth]));
    ...
    for (NSInteger index = startIndex > 0 ? startIndex - 1 : 0; index < arrCount; ++index) {
        SZKLinePositionModel *kLinePositionModel = self.needDrawKLinePositionModels[index];
        
        CGFloat minX = kLinePositionModel.HighPoint.x - ([SZKLineGlobalVariable kLineGap] + [SZKLineGlobalVariable kLineWidth]/2);
        CGFloat maxX = kLinePositionModel.HighPoint.x + ([SZKLineGlobalVariable kLineGap] + [SZKLineGlobalVariable kLineWidth]/2);
...
- (NSArray *)private_extractNeedDrawModels{
    CGFloat lineGap = [SZKLineGlobalVariable kLineGap];
    CGFloat lineWidth = [SZKLineGlobalVariable kLineWidth];
    ...
    //DLog(@"这是模型开始的index-----------%lu",needDrawKLineStartIndex);
...
- (NSArray *)private_convertToKLinePositionModelWithKLineModels {
    ...
    [kLineModels enumerateObjectsUsingBlock:^(SZKLineModel * _Nonnull kLineModel, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if (_targetLineStatus == SZKLineTargetLineStatusBOLL) {
    ...
    CGFloat minY = SZKLineMainViewMinY;
    CGFloat maxY = self.parentScrollView.frame.size.height * [SZKLineGlobalVariable kLineMainViewRadio] - 15;
    ...
    {
        //K线坐标转换
        SZKLineModel *kLineModel = kLineModels[idx];
        
        CGFloat xPosition = self.startXPosition + idx * ([SZKLineGlobalVariable kLineWidth] + [SZKLineGlobalVariable kLineGap]);
        ...
        if(ABS(closePointY - openPoint.y) < SZKLineMinWidth)
        {
            if(openPoint.y > closePointY)
            {
                openPoint.y = closePointY + SZKLineMinWidth;
            } else if(openPoint.y < closePointY)
            {
                closePointY = openPoint.y + SZKLineMinWidth;
            } else {
                if(idx > 0)
                {
                    SZKLineModel *preKLineModel = kLineModels[idx-1];
                    ...
                    //idx==0即第一个时
                    SZKLineModel *subKLineModel = kLineModels[idx+1];
        ...
        SZKLinePositionModel *kLinePositionModel = [SZKLinePositionModel modelWithOpen:openPoint close:closePoint high:highPoint low:lowPoint];
...
- (void)private_addAllEventListener{
    ...
    [_parentScrollView addObserver:self forKeyPath:SZKLineContentOffsetKey options:NSKeyValueObservingOptionNew context:observerContext];
}
#pragma mark - setter,getter方法
- (NSInteger)startXPosition{
    NSInteger leftArrCount = self.needDrawStartIndex;
    CGFloat startXPosition = (leftArrCount + 1) * [SZKLineGlobalVariable kLineGap] + leftArrCount * [SZKLineGlobalVariable kLineWidth] + [SZKLineGlobalVariable kLineWidth]/2;
...
- (NSInteger)needDrawStartIndex{
    ...
    NSUInteger leftArrCount = ABS(scrollViewOffsetX - [SZKLineGlobalVariable kLineGap]) / ([SZKLineGlobalVariable kLineGap] + [SZKLineGlobalVariable kLineWidth]);
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSString *,id> *)change context:(void *)context{
    if([keyPath isEqualToString:SZKLineContentOffsetKey])
    {
        CGFloat difValue = ABS(self.parentScrollView.contentOffset.x - self.oldContentOffsetX);
        if(difValue >= [SZKLineGlobalVariable kLineGap] + [SZKLineGlobalVariable kLineWidth])
        {
...
- (void)removeAllObserver{
    [_parentScrollView removeObserver:self forKeyPath:SZKLineContentOffsetKey context:observerContext];
}
