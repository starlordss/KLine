...
@interface SZKLineAccessoryView ()
...
@implementation SZKLineAccessoryView
...
- (void)drawRect:(CGRect)rect
{
     ...
     */
    if(self.targetLineStatus != SZKLineTargetLineStatusKDJ)
    {
        /**
         MACD
         */
        SZKLineAccessory *kLineAccessory = [[SZKLineAccessory alloc]initWithContext:context];
        [self.needDrawKLineAccessoryPositionModels enumerateObjectsUsingBlock:^(SZKLineVolumePositionModel * _Nonnull volumePositionModel, NSUInteger idx, BOOL * _Nonnull stop) {
...
{
    CGFloat minY = SZKLineAccessoryViewMinY;
    CGFloat maxY = SZKLineAccessoryViewMaxY;
    ...
    if(self.targetLineStatus != SZKLineTargetLineStatusKDJ)
    {
        ...
        [kLineModels enumerateObjectsUsingBlock:^(SZKLineModel *  _Nonnull model, NSUInteger idx, BOOL * _Nonnull stop) {
            ...
            CGFloat yPosition = -(model.MACD.floatValue - 0)/unitValue + SZKLineAccessoryViewMiddleY;
            
            //            CGFloat yPosition = ABS(minY + (model.MACD.floatValue - minValue)/unitValue);
            
            CGPoint startPoint = CGPointMake(xPosition, yPosition);
            
            CGPoint endPoint = CGPointMake(xPosition,SZKLineAccessoryViewMiddleY);
            ...
            if(unitValue > 0.0000001)
            {
                if(model.DIF)
                {
                    DIFY = -(model.DIF.floatValue - 0)/unitValue + SZKLineAccessoryViewMiddleY;
            ...
            if(unitValue > 0.0000001)
            {
                if(model.DEA)
                {
                    DEAY = -(model.DEA.floatValue - 0)/unitValue + SZKLineAccessoryViewMiddleY;
