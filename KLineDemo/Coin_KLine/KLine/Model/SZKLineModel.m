...
- (NSNumber *)MA7
{
    if([SZKLineGlobalVariable isEMALine] == SZKLineTargetLineStatusMA)
    {
        if (!_MA7) {
...
- (SZKLineModel *)PreviousKlineModel
{
    if (!_PreviousKlineModel) {
        _PreviousKlineModel = [SZKLineModel new];
...
- (SZKLineGroupModel *)ParentGroupModel
{
    if(!_ParentGroupModel) {
        _ParentGroupModel = [SZKLineGroupModel new];
    }
    return _ParentGroupModel;
}
//对Model数组进行排序，初始化每个Model的最新9Clock的最低价和最高价
- (void)rangeLastNinePriceByArray:(NSArray<SZKLineModel *> *)models condition:(NSComparisonResult)cond
{
    switch (cond) {
            //最高价
